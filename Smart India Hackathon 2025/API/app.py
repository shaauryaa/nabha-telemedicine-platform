...
JWT_SECRET = os.environ.get('JWT_SECRET', 'nabha-medicine-tracker-secret-key')
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            current_user = query_db("SELECT * FROM pharmacists WHERE id = ?", (data['user_id'],), one=True)
            if not current_user:
...
def init_db():
    ...
    # Create pharmacists table if it doesn't exist
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute('''
...
@app.route("/api/pharmacist/register", methods=["POST"])
def register_pharmacist():
    """Register a new pharmacist and create their pharmacy"""
    try:
        data = request.json
        ...
        pharmacy_location = data.get("pharmacy_location", "").strip()
        pharmacy_address = data.get("pharmacy_address", "").strip()
        phone = data.get("phone", "").strip()
        
        # Validate required fields
        if not all([name, email, password, pharmacy_name, pharmacy_location]):
            return jsonify({"error": "Name, email, password, pharmacy name, and pharmacy location are required"}), 400
        
        # Check if email already exists
        existing = query_db("SELECT id FROM pharmacists WHERE email = ?", (email,), one=True)
        if existing:
        ...
        existing_pharmacy = query_db(
            ...
            (pharmacy_name, pharmacy_location), one=True
        )
        ...
        # Create pharmacy and pharmacist in a transaction
        conn = sqlite3.connect(DB)
        cursor = conn.cursor()
        ...
        try:
            ...
            cursor.execute(
                """INSERT INTO pharmacies (name, location, address, phone) 
                   VALUES (?, ?, ?, ?)""",
                (pharmacy_name, pharmacy_location, pharmacy_address, phone)
            )
            pharmacy_id = cursor.lastrowid
            
            # Then, create the pharmacist linked to this pharmacy
            cursor.execute(
                """INSERT INTO pharmacists (name, email, password, pharmacy_id, phone) 
                   VALUES (?, ?, ?, ?, ?)""",
                (name, email, hashed_password, pharmacy_id, phone)
            )
        ...
        return jsonify({
            "status": "success",
            "message": "Pharmacist and pharmacy registered successfully",
            "token": token,
            "pharmacist": {
                "id": pharmacist_id,
                "name": name,
...
def login_pharmacist():
    ...
    try:
        ...
        # Get pharmacist
        pharmacist = query_db(
            """SELECT p.*, ph.name as pharmacy_name, ph.location as pharmacy_location 
               FROM pharmacists p 
        ...
        # Check password
        if not bcrypt.checkpw(password.encode('utf-8'), pharmacist[3].encode('utf-8')):
            return jsonify({"error": "Invalid credentials"}), 401
        
        # Generate JWT token
        token = jwt.encode({
            'user_id': pharmacist[0],
            'email': pharmacist[2],
            'role': 'pharmacist',
            'pharmacy_id': pharmacist[4]
        }, JWT_SECRET, algorithm='HS256')
        ...
        return jsonify({
            ...
            "pharmacist": {
                ...
                "pharmacy_id": pharmacist[4],
                "phone": pharmacist[5],
                "pharmacy_name": pharmacist[7],
                "pharmacy_location": pharmacist[8]
            }
        })
...
@app.route("/api/pharmacist/profile", methods=["GET"])
@token_required
def get_pharmacist_profile(current_user):
    ...
    return jsonify({
        "pharmacist": {
            ...
            "pharmacy_id": current_user[4],
            "phone": current_user[5],
...
@token_required
def get_pharmacist_inventory(current_user):
    """Get inventory for logged-in pharmacist's pharmacy"""
    try:
        pharmacy_id = current_user[4]
        if not pharmacy_id:
            return jsonify({"error": "No pharmacy assigned to this pharmacist"}), 400
        
        # Get pharmacy info
        pharmacy = query_db("SELECT name, location FROM pharmacies WHERE id = ?", (pharmacy_id,), one=True)
        if not pharmacy:
...
@app.route("/api/pharmacist/update-stock", methods=["POST"])
@token_required
def update_pharmacist_stock(current_user):
    """Update medicine stock for logged-in pharmacist's pharmacy"""
    try:
        pharmacy_id = current_user[4]
        if not pharmacy_id:
            return jsonify({"error": "No pharmacy assigned to this pharmacist"}), 400
        ...
        if not all([medicine_name, quantity is not None]):
            return jsonify({"error": "medicine_name and quantity are required"}), 400
...
if __name__ == "__main__":
    ...
    print(f"User Dashboard: http://localhost:{port}")
    print(f"Pharmacy Dashboard: http://localhost:{port}/pharmacy")
